...
if(isset($_SESSION['typeProfilModif']) and $_SESSION['typeProfilModif'] === 'candidat' and isModifiableCandidat() and isConnecter())
{
	...
	$req1->bindValue(':id_pers', $_SESSION['id']);
	...
	$req2->bindValue(':id_pers',$_SESSION['id']);

	$req2->execute();

	$req3 = $requeteur->getRequete('DELETE FROM qualite WHERE num_cand = :num_cand');
	$req3->bindValue(':num_cand', $_SESSION['idProfil']);
	$req3->execute();
	...
	foreach ($_POST['qualite'] as  $value) 
	{
		...
		$req4->bindValue(':numCandidat', $_SESSION['idProfil']);
		$req4->execute();
	}
				...
				<form id="insc" method="post" action="ModifierProfil.php">
					<div class="row">
						...
						<div>
							...
							<?php 
							$date = explode('-', $val['dateNaissance']);
							?>
							<div class="form-group col-xs-4" id="blackcolor">
								<div class="label label-primary">
									Jour de naissance:
								</div>
								<select name="jour" class="form-control">
									...
									for($i=1; $i <= 31; $i++){
										echo '<option value ="'.$i.'"';
										if($i===intval($date[2])){echo ' selected';}
										echo '>'.$i.'</option>';
									}
									?>
								</select>
							...
							<div class="form-group col-xs-4" id="blackcolor">
								...
								<select name="mois" class="form-control">
									...
									for($i=1; $i <= 12; $i++){
										echo '<option value ="'.$i.'"';
										if($i===intval($date[1])){echo ' selected';}
										echo '>'.$i.'</option>';									}
										?>
									</select>
								</div>
								<div class="form-group col-xs-4" id="blackcolor">
									...
									<select name="annee" class="form-control ">
										...
										for($i=1900; $i <= 2017; $i++){
											echo '<option value ="'.$i.'"';
											if($i===intval($date[0])){echo ' selected';}
											echo '>'.$i.'</option>';										}
											?>
										</select>
									</div>
